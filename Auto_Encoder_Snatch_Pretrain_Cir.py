...
class AutoEncoder(object):
    def __init__(self, trainable):
        ...
        self.lamda_appr = 43
        self.lamda_pose = 43
        self.logs_dir = "./Naive2_"+str(self.lamda_recon)+"_"+str(self.lamda_appr)+"_"+str(self.lamda_pose)+"_Result_Normalize_Cir"
    ...
    def construct_network(self):
        self.input_image1 = tf.placeholder(tf.float32, [self.batch_size,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="input_imagec1")
        self.input_image2 = tf.placeholder(tf.float32, [self.batch_size,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="input_imagec2")

        self.selected_image1_1 = tf.placeholder(tf.float32, [1,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="selected_imagec1_1")
        self.selected_image2_1 = tf.placeholder(tf.float32, [1,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="selected_imagec2_1")
        self.selected_image1_2 = tf.placeholder(tf.float32, [1,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="selected_imagec1_2")
        self.selected_image2_2 = tf.placeholder(tf.float32, [1,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="selected_imagec2_2")
         
        self.hidden_state_pose1 = self._encoder(self.input_image1, scope_name = 'encoder_posec',trainable = self.trainable)
        self.hidden_state_pose2 = self._encoder(self.input_image2, scope_name = 'encoder_posec',trainable = self.trainable, scope_reuse = True)
        self.hidden_state_appr1_1 = self._encoder(self.selected_image1_1, scope_name = 'encoder_apprc',trainable = self.trainable)
        self.hidden_state_appr2_1 = self._encoder(self.selected_image2_1, scope_name = 'encoder_apprc',trainable = self.trainable, scope_reuse = True)
        self.hidden_state_appr1_2 = self._encoder(self.selected_image1_2, scope_name = 'encoder_apprc',trainable = self.trainable, scope_reuse = True)
        self.hidden_state_appr2_2 = self._encoder(self.selected_image2_2, scope_name = 'encoder_apprc',trainable = self.trainable, scope_reuse = True)
        ...
        self.appr_variables = [v for v in self.train_variables if v.name.startswith("encoder_apprc")]
        self.pose_variables = [v for v in self.train_variables if v.name.startswith("encoder_posec")]
        tf.summary.scalar("reconc_loss", self.recon_loss)
        tf.summary.scalar("apprc_loss", self.appr_loss)
        tf.summary.scalar("posec_loss", self.pose_loss)
    ...
    def train(self):
        ...
        for itr in range(self.iterations):
            ...
            for i in range(self.batch_size):
                cur_img = np.float32(misc.imread(self.images_dir+'Train/'+str(cur_dir2)+'/'+str(i+1)+'_cir.jpg'))
                cur_img -= 127.5
                cur_img /= 127.5
            ...
            _, recon_loss_eval = self.sess.run([self.recon_op,self.recon_loss],\
                feed_dict={self.input_image1:img_batch1,self.input_image2:img_batch2,\
                           self.selected_image1_1:[img_batch1[cur_pos1]],self.selected_image1_2:[img_batch1[cur_pos2]],\
                           self.selected_image2_1:[img_batch2[cur_pos1]],self.selected_image2_2:[img_batch2[cur_pos2]]})
            print str(itr)+'reconc: '+str(recon_loss_eval)+'apprc: '+str(appr_loss_eval)+'posec: '+str(pose_loss_eval)
            if itr % 100 == 99:
                ...
                for i in range(self.batch_size):
					cur_img = np.float32(misc.imread(self.images_dir+'Val/'+str(cur_dir1)+'/'+str(i+1)+'_cir.jpg'))
					cur_img -= 127.5
					cur_img /= 127.5
					img_batch1[i] = cur_img
                for i in range(self.batch_size):
					cur_img = np.float32(misc.imread(self.images_dir+'Val/'+str(cur_dir2)+'/'+str(i+1)+'_cir.jpg'))
					cur_img -= 127.5
					cur_img /= 127.5
					img_batch2[i] = cur_img
                hidden_state_pose_eval1,hidden_state_pose_eval2,\
