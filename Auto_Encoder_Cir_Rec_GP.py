...
class AutoEncoder(object):
    def __init__(self, trainable):
        ...
        self.lamda_appr = 43
        self.lamda_pose = 43
        self.logs_dir = "./Naive2_Cir_Rec_GP"
    ...
    def train(self):
        ...
        gpdm_model = gpdm.GPDM(np.squeeze(hidden_state_pose_eval1),3)
        linit = gpdm_model.compute_log_likelihood()
        for itr in range(self.iterations):
            ...
            if True:
                ...
                for i in range(self.batch_size):
                    for j in range(3):
                       fout.write(str(gpdm_model.X.value[i,j])+' ')
                    fout.write('\n')
                fout.close()
                #np.savetxt("X.txt",gpdm_model.X.value) 
        '''
