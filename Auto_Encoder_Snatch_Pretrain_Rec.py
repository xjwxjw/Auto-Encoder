...
class AutoEncoder(object):
    def __init__(self, trainable):
        ...
        self.lamda_appr = 43
        self.lamda_pose = 43
        self.logs_dir = "./Naive2_"+str(self.lamda_recon)+"_"+str(self.lamda_appr)+"_"+str(self.lamda_pose)+"_Result_Normalize_Rec"
    ...
    def construct_network(self):
        self.input_image1 = tf.placeholder(tf.float32, [self.batch_size,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="input_imager1")
        self.input_image2 = tf.placeholder(tf.float32, [self.batch_size,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="input_imager2")

        self.selected_image1_1 = tf.placeholder(tf.float32, [1,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="selected_imager1_1")
        self.selected_image2_1 = tf.placeholder(tf.float32, [1,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="selected_imager2_1")
        self.selected_image1_2 = tf.placeholder(tf.float32, [1,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="selected_imager1_2")
        self.selected_image2_2 = tf.placeholder(tf.float32, [1,self.IMAGE_HEIGHT,self.IMAGE_WIDTH,3], name="selected_imager2_2")
         
        self.hidden_state_pose1 = self._encoder(self.input_image1, scope_name = 'encoder_poser',trainable = self.trainable)
        self.hidden_state_pose2 = self._encoder(self.input_image2, scope_name = 'encoder_poser',trainable = self.trainable, scope_reuse = True)
        self.hidden_state_appr1_1 = self._encoder(self.selected_image1_1, scope_name = 'encoder_apprr',trainable = self.trainable)
        self.hidden_state_appr2_1 = self._encoder(self.selected_image2_1, scope_name = 'encoder_apprr',trainable = self.trainable, scope_reuse = True)
        self.hidden_state_appr1_2 = self._encoder(self.selected_image1_2, scope_name = 'encoder_apprr',trainable = self.trainable, scope_reuse = True)
        self.hidden_state_appr2_2 = self._encoder(self.selected_image2_2, scope_name = 'encoder_apprr',trainable = self.trainable, scope_reuse = True)
        ...
        self.appr_variables = [v for v in self.train_variables if v.name.startswith("encoder_apprr")]
        self.pose_variables = [v for v in self.train_variables if v.name.startswith("encoder_poser")]
        tf.summary.scalar("reconr_loss", self.recon_loss)
        tf.summary.scalar("apprr_loss", self.appr_loss)
        tf.summary.scalar("poser_loss", self.pose_loss)
    ...
    def train(self):
        ...
        for itr in range(self.iterations):
            ...
            cur_dir2 = np.random.choice(100)+1
            ...
            for i in range(self.batch_size):
                cur_img = np.float32(misc.imread(self.images_dir+'Train/'+str(cur_dir2)+'/'+str(i+1)+'_rec.jpg'))
                cur_img -= 127.5
                cur_img /= 127.5
            ...
            _, recon_loss_eval = self.sess.run([self.recon_op,self.recon_loss],\
                feed_dict={self.input_image1:img_batch1,self.input_image2:img_batch2,\
                           self.selected_image1_1:[img_batch1[cur_pos1]],self.selected_image1_2:[img_batch1[cur_pos2]],\
                           self.selected_image2_1:[img_batch2[cur_pos1]],self.selected_image2_2:[img_batch2[cur_pos2]]})
            print str(itr)+'reconr: '+str(recon_loss_eval)+'apprr: '+str(appr_loss_eval)+'poser: '+str(pose_loss_eval)
            if itr % 100 == 99:
                ...
                while (cur_pos2 == cur_pos1):
		            cur_pos2 = np.random.choice(self.batch_size)
                print "val1:",cur_dir1,cur_pos1
                print "val2:",cur_dir2,cur_pos2
                for i in range(self.batch_size):
		            cur_img = np.float32(misc.imread(self.images_dir+'Val/'+str(cur_dir1)+'/'+str(i+1)+'_rec.jpg'))
		            cur_img -= 127.5
		            cur_img /= 127.5
		            img_batch1[i] = cur_img
                for i in range(self.batch_size):
		            cur_img = np.float32(misc.imread(self.images_dir+'Val/'+str(cur_dir2)+'/'+str(i+1)+'_rec.jpg'))
		            cur_img -= 127.5
		            cur_img /= 127.5
		            img_batch2[i] = cur_img
                hidden_state_pose_eval1,hidden_state_pose_eval2,\
                ...
                feed_dict={self.input_image1:img_batch1,self.input_image2:img_batch2,\
                           self.selected_image1_1:[img_batch1[cur_pos1]],self.selected_image1_2:[img_batch1[cur_pos2]],\
                           self.selected_image2_1:[img_batch2[cur_pos1]],self.selected_image2_2:[img_batch2[cur_pos2]]})
                self._save_images(img_batch1,"/val/ori1_"+str(0))
                self._save_images(gen_images_eval1,"/val/gen1_"+str(0))
                self._save_images(img_batch2,"/val/ori2_"+str(0))
                self._save_images(gen_images_eval2,"/val/gen2_"+str(0))
    ...
    def test(self):
        ...
        threads = tf.train.start_queue_runners(self.sess, coord)
        for cur in range(1):
            cur_dir1 = 1#np.random.choice(100)+1
            cur_dir2 = 2#np.random.choice(100)+1
            while (cur_dir2 == cur_dir1):
	            cur_dir2 = np.random.choice(100)+1
            cur_pos1 = np.random.choice(self.batch_size)
            cur_pos2 = np.random.choice(self.batch_size)
            ...
            if not os.path.exists(os.path.join(self.logs_dir,'train')):
                os.mkdir(os.path.join(self.logs_dir,'train'))
            self._save_images(img_batch1,"/train/ori1_"+str(cur))
            self._save_images(gen_images_eval1,"/train/gen1_"+str(cur))
            self._save_images(img_batch2,"/train/ori2_"+str(cur))
            self._save_images(gen_images_eval2,"/train/gen2_"+str(cur))
            fout = open(self.logs_dir+'/hidden_state_pose1.txt','w')
            for i in range(self.batch_size):
                for j in range(512):
                   fout.write(str(hidden_state_pose_eval1[i][0][0][j])+' ')
